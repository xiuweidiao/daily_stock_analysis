...
- Unreleased: add a state-driven close watchdog that independently recovers missing/stale/invalid completed-session snapshots, shares close concurrency with the normal workflow, and verifies the final remote contract.
...
- [新功能] 新增独立的 A 股持仓免费行情 JSON 管道与盘前、午盘、盘后定时工作流。
- [修复] 为 A 股持仓管道增加手动盘中快照和收盘时间保护，并校正量比、供应商时间与抓取时间语义。
